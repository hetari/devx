...
import { Loader2, ShieldCheck, ShieldAlert, Save, RefreshCcw } from 'lucide-vue-next';
...
const trustClass = (rate: number, sample: number) => {
  ...
  if (rate >= 0.85) return 'bg-emerald-500/20 text-emerald-200 border-emerald-400/30';
...
<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      ...
      <Button variant="ghost" size="sm" :disabled="loading" class="gap-2" @click="loadBudgets">
    ...
    <p class="text-sm text-muted-foreground" dir="rtl">
      كل عضو في المجلس يبدأ بصلاحيات محدودة. كلما وافقت على إجراءاته، ترتفع نسبة الثقة، ويمكنك توسيع ميزانيته.
    </p>

    <div v-if="error" class="rounded-xl border border-destructive/40 bg-destructive/10 p-3 text-sm" dir="rtl">
      {{ error }}
    </div>

    <div v-for="block in blocks" :key="block.agent" class="rounded-3xl border border-white/10 bg-card/40 backdrop-blur-md p-5 shadow-xl">
      <!-- Agent header -->
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3" dir="rtl">
          <AgentAvatar :agent="block.agent" state="idle" size="sm" :show-label="false" />
          <div>
            <div class="text-base font-bold">{{ getAgent(block.agent).arabicName }}</div>
            <div class="text-xs opacity-60">{{ getAgent(block.agent).domain }}</div>
          </div>
        </div>
        <div :class="['rounded-full border px-3 py-1.5 text-xs font-bold flex items-center gap-2', trustClass(block.approval.rate, block.approval.sample)]" dir="rtl">
          <ShieldAlert v-if="block.approval.sample === 0" class="size-3.5" />
          <ShieldCheck v-else class="size-3.5" />
          <span>{{ trustLabel(block.approval.rate, block.approval.sample) }}</span>
          <span v-if="block.approval.sample > 0" class="opacity-80">
            ({{ Math.round(block.approval.rate * 100) }}% / {{ block.approval.sample }} قرار)
          </span>
        </div>
      </div>
      ...
      <div class="space-y-3">
        ...
        >
          <div class="flex items-center justify-between flex-wrap gap-2">
            <div class="font-bold text-sm">{{ actionTypeLabel(row.actionType) }}</div>
            <div class="flex items-center gap-2 text-xs">
              <label class="flex items-center gap-1 cursor-pointer">
                <input v-model="row.enabled" type="checkbox" class="size-4 accent-primary" />
          ...
          <div class="grid gap-3 md:grid-cols-3">
            <div>
              <div class="text-[10px] uppercase tracking-wider opacity-60 mb-1">حد العدد اليومي</div>
              <input
            ...
            <div>
              <div class="text-[10px] uppercase tracking-wider opacity-60 mb-1">حد المبلغ اليومي ($)</div>
              <input
            ...
            <div>
              <div class="text-[10px] uppercase tracking-wider opacity-60 mb-1">يطلب موافقتك أعلى من ($)</div>
              <input
          ...
          <div class="flex justify-end">
            <Button
              ...
              @click="saveRow(row)"
            >
              <Loader2 v-if="saving === `${row.agent}:${row.actionType}`" class="size-3 animate-spin" />
        ...
        <div v-if="!block.budgets.length" class="rounded-2xl border border-white/10 bg-background/30 p-4 text-sm opacity-70" dir="rtl">
          لا توجد ميزانيات بعد لهذا العضو.
        </div>
      </div>
    </div>

    <div v-if="!loading && !blocks.length" class="rounded-2xl border border-white/10 bg-background/30 p-8 text-center text-sm opacity-70" dir="rtl">
      لا توجد بيانات.
    </div>
  </div>
</template>
