...
const { previewData, handleConfirmPreview, handleCancelPreview } =
  useAIAssistant();
...
<template>
  <div
    class="rounded-2xl border border-white/10 bg-card/40 p-4 flex items-center gap-3"
  >
    ...
    <Dialog v-model:open="isPreviewOpen">
      <DialogContent
        class="sm:max-w-[500px] border-white/10 bg-card/95 backdrop-blur-xl max-h-[90vh] flex flex-col p-0 overflow-hidden"
      >
        <DialogHeader class="p-6 pb-2">
          <DialogTitle class="text-xl font-black tracking-tight"
            >تأكيد وتعديل البيانات</DialogTitle
          >
          <DialogDescription>
        ...
        <div
          v-if="previewData"
          class="flex-1 overflow-y-auto p-6 pt-2 space-y-8"
        >
          <!-- AI Summary & Date Edit -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-2 space-y-2">
              <Label
                class="text-[10px] font-black uppercase tracking-[0.2em] text-primary block px-1"
                >ملخص الذكاء الاصطناعي</Label
              >
              <Textarea
          ...
          <div v-if="previewData.revenues.length > 0" class="space-y-4">
            <div class="flex items-center gap-2">
              ...
              <h5
                class="text-[10px] font-black uppercase tracking-[0.2em] text-chart-4"
              >
                الإيرادات / الدخل
              </h5>
              <div class="h-px flex-1 bg-chart-4/20"></div>
            ...
            <div class="space-y-4">
              <div
                ...
                :key="i"
                class="p-4 rounded-xl bg-white/5 border border-white/5 space-y-3 group relative"
              >
                <button
                  @click="removeRevenue(i)"
                  class="absolute top-2 start-2 p-1.5 text-white/20 hover:text-destructive transition-colors opacity-0 group-hover:opacity-100"
                >
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="col-span-2 space-y-1.5">
                    <Label
                      class="text-[10px] opacity-60 uppercase tracking-wider px-1"
                      >العنوان</Label
                    >
                    <Input
                      v-model="r.title"
                  ...
                  <div class="space-y-1.5">
                    ...
                    <Input
                      ...
                      type="number"
                      class="bg-white/5 border-white/10 font-mono"
                    />
                  </div>
                  <div class="space-y-1.5">
                    <Label
          ...
          <div v-if="previewData.expenses.length > 0" class="space-y-4">
            <div class="flex items-center gap-2">
              ...
              <h5
                class="text-[10px] font-black uppercase tracking-[0.2em] text-destructive"
              >
                المصاريف / التكاليف
              </h5>
              <div class="h-px flex-1 bg-destructive/20"></div>
        ...
        <DialogFooter
          class="p-6 pt-2 flex flex-row gap-3 sm:justify-start bg-card/50 backdrop-blur-md border-t border-white/5"
        >
          <Button
            variant="outline"
            class="flex-1 rounded-xl border-white/10 hover:bg-white/5"
            @click="handleCancelPreview"
          >
            <XCircle class="size-4 ms-2" /> إلغاء
          </Button>
          <Button
            class="flex-1 rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20"
            @click="handleConfirmPreview"
          >
            <CheckCircle2 class="size-4 ms-2" /> تأكيد وحفظ
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
