...
const hasDisagreement = computed(() => (props.stance.disagreesWith?.length ?? 0) > 0);
</script>
...
<template>
  <div
    :class="[
      ...
      tintBorder,
      isSpeaking ? 'scale-105 ring-2 ring-white/30 -translate-y-2' : 'scale-100',
    ]"
  >
    <AgentAvatar
    ...
    >
      <span dir="rtl">وافق على رأي {{ agent.arabicName }}</span>
    </Button>
  </div>
</template>
